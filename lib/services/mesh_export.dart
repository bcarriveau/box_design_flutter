...
/// [thicknessMm], with every screw/zip-tie hole *and* every hole already
/// baked into the box template itself (e.g. a real enclosure's own mounting
/// flange holes) cut all the way through. Placed controller/power-supply
/// templates are not represented in the 3D geometry — they mark where
/// components mount on top of the plate, not material to remove.
Mesh buildPlateMesh(BoxProject project, {required double thicknessMm}) {
  final split = _splitBoxOutline(project.boxOutline);
  return extrudePlate(
    outer: split.outer,
    holes: [...split.ownHoles, ..._holeBoundaries(project.holes)],
